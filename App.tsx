...
//const KEY_GPT = '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';
  ...
  async function handleGenerate() {
    ...
    fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.20, //quanto mais próximo de 0, mais preciso é
        max_tokens: 500,
        top_p: 1,
      })
    })
      ...
      .then((data) => {
        ...
        if (data.choices && data.choices.length > 0 && data.choices[0].message) {
          console.log("Generated travel:", data.choices[0].message.content);
          setTravel(data.choices[0].message.content);
        } else {
          console.error("Resposta inesperada da API:", data);
          Alert.alert("Erro", "Erro ao processar a resposta da API.");
        }
      })
      
      .catch((error) => {
        console.error("Erro na requisição:", error);
        Alert.alert("Erro", "Erro na requisição à API. " + (error.message || ""));
      })
